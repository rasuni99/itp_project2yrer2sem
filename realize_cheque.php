                ...
                <div class="example-modal">
                        <div class="modal modal-warning">
                            <div class="modal-dialog">
                                <form action = "realize_cheque.php" class="form-horizontal" method="POST"  enctype="multipart/form-data" name="form" id="form">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">Are you sure you want to realize this cheque ?</h4>
                                        </div>
                                        ...
                                        <div class="modal-footer">
                                           <button type="button" class="btn btn-outline pull-left"  onClick="parent.location = 'pending_realizing.php'">Cancel</button>
                                            <button type="submit" name='realize' class="btn btn-outline" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Please Wait">Realize</button>
                                        </div>
                                    </div>
                                    <!-- /.modal-content -->
                                </form>
...
if (isset($_POST['realize'])) {
    ...
    $sql8 = "SELECT id FROM cheque WHERE realize='1' AND id='$id' ";
    $result=mysqli_query($con,$sql8);
    $row_count = mysqli_num_rows($result);
    
    if($row_count>0){
        
        echo "<script>window.location = 'pending_realizing.php?msge=CHEQUE IS ALREADY REALIZED ! ( YOU ARE CLICKING TOO FAST ! ) ';</script>";
        
    }
    else{
    
                $sql = "UPDATE cheque SET realize='1' WHERE id='$id' ";



                if (mysqli_query($con, $sql)) {

                 $sql1 = "UPDATE cash_book SET payment_received='1' WHERE payment_type_id='$id' ";    
...
//                 mysqli_query($con, $sql2);
                ...
                $sql11 ="INSERT INTO user_activity (user,activity) VALUES ('$user','REALIZED CHEQUE ID :$id ')";
                                                                            mysqli_query($con, $sql11);

                echo "<script>window.location = 'pending_realizing.php?msg=CHEQUE IS REALIZED ! CREDIT HAS BEEN RESOLVED ';</script>";
