...
if (isset($_POST['invoiceno'])) {
    ...
    $invoice_id = 0;
    $sql1 = "SELECT id FROM invoice_return WHERE return_number='$invoice_no'";
 ...
 $sql1 = "SELECT name,phone,address,email,description,br_no,cheques_payable FROM company WHERE id='$company'";
    ...
    $sql1 = "SELECT id,return_number,net_amount,type,customer_id,date AS date FROM invoice_return WHERE id='$invoice_id'";
    $result1 = mysqli_query($con, $sql1);
        while ($arraySomething1 = mysqli_fetch_array($result1)) {
        $invoice_id = $arraySomething1['id'];
...
<?php
            ...
            echo "Failed : Wrong Return Note NO. Check the Return Note No and Try again";  
        }
